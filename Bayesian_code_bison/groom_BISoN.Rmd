---
title: "Run BISoN on cayo grooming data"
output:
  rmarkdown::github_document
Author: C. Testard (adapted from J. Hart and D. DeMoor)
Date: April 2022
---
...
### Load and format cayo groom data ###

#Load data
dataPath = '~/Documents/GitHub/Cayo-Maria-Survival/Data All Cleaned/BehavioralDataFiles/'
    groom_data = read.csv(paste(dataPath,"GroupV2019_GroomingEvents.txt", sep=""))
    meta_data = read.csv(paste(dataPath,"GroupV2019_GroupByYear.txt", sep=""))

## GENERATE OBSERVATION-LEVEL DATA INPUT ##
df_obs <- groom_data
df_obs$event = 1
names(df_obs) = c("ID1","ID2","duration","observation_name","event")
df_obs$ID1 = as.factor(df_obs$ID1); df_obs$ID2 = as.factor(df_obs$ID2)
df_obs$dyad_id = as.factor(paste(df_obs$ID1, df_obs$ID2,sep='.'))

#add id qualifiers from meta data df
idx_ID1 = match(df_obs$ID1, meta_data$id); idx_ID2 = match(df_obs$ID2, meta_data$id)
df_obs$ID1_sex = meta_data$sex[idx_ID1]
df_obs$ID2_sex = meta_data$sex[idx_ID2]
df_obs$ID1_rank = meta_data$ordinal.rank[idx_ID1]
df_obs$ID2_rank = meta_data$ordinal.rank[idx_ID2]
df_obs$ID1_age = meta_data$age[idx_ID1]
df_obs$ID2_age = meta_data$age[idx_ID2]
df_obs$ID1_obseff = meta_data$hrs.focalfollowed[idx_ID1]
df_obs$ID2_obseff = meta_data$hrs.focalfollowed[idx_ID2]
df_obs$group = "V"; df_obs$year = 2019; df_obs$isPost = "post"
df_obs$total_obs_time = df_obs$ID1_obseff + df_obs$ID2_obseff

head(df_obs)

#NOTES: time of day or time of year can be deduced by the observation name
# Computationally it's easier to work with observation-level factors using IDs rather than factor names in the statistical model, so we'll map the factors to factor IDs before we put the data into the model. I.e. assigning a numerical ID to each observation-level factor (AM-->1, PM-->2)
```

```{r}
## GENERATE AGGREGATE DATA INPUT ##

# Output the Master Edgelist of all possible pairs given the unique IDs (directional because grooming).
unqIDs = meta_data$id
df_obs_agg = calcMasterEL_groom(unqIDs); names(df_obs_agg)[4]="total_event_time"
groom_data$conc = paste(groom_data$groom_giver,groom_data$groom_reciever,sep=".")
for (ii in 1:length(df_obs_agg$conc)){
  df_obs_agg$total_event_time[ii] = sum(groom_data$constrained_duration
                                        [which(groom_data$conc == df_obs_agg$conc[ii])], na.rm=T)
  df_obs_agg$num_events[ii] = length(which(groom_data$conc == df_obs_agg$conc[ii]))
}
#Get total hours followed for each dyad
df_obs_agg$ID1_obseff_duration = meta_data$hrs.focalfollowed[match(df_obs_agg$givingID, meta_data$id)]
df_obs_agg$ID1_obseff_samples = NA
df_obs_agg$ID2_obseff_duration = meta_data$hrs.focalfollowed[match(df_obs_agg$receivingID, meta_data$id)] 
df_obs_agg$ID2_obseff_samples = NA
...
## Add id qualifiers
names(df_obs_agg)[1:3] = c("ID1","ID2","dyad_id")
...
df_obs_agg$dyad_id = as.integer(as.factor(df_obs_agg$dyad_id))
#sex
...
df_obs_agg$group = "V"; df_obs_agg$year = 2019; df_obs_agg$isPost = "post"
...
# Plot the density of the observed event durations
...
for (dyad_id in 1:model_data$M) {
  ...
  adj_tensor[dyad_row$ID1_id, dyad_row$ID2_id, ] <- logit_p_samples[, dyad_id]
}
