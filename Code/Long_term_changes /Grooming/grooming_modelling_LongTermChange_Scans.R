#grooming_modelling_LongTermChange.R
...
mdl.groomPrePost.scans<-lmer( std.dens~ 1+ isPost.year + (1|group) , data=density.all)
summary(mdl.groomPrePost.scans)

#Groom strength
mdl.strength.groomPrePost.scans <- with(imp.prepost, lmer(std.groom.strength~ 1+ isPost.year +age+ sex +(1|group) + (1|id)))
summary(mice::pool(mdl.strength.groomPrePost.scans ))
...
#Groom degree
mdl.degree.groomPrePost.scans <- with(imp, lmer(std.groom.degree~ 1+ isPost.year + sex +age+
                                                  (1|group) + (1|id)))
summary(mice::pool(mdl.degree.groomPrePost.scans))
...
setwd("~/Documents/GitHub/Cayo-Maria-Survival/Data/R.Data/")
save(mdl.groomPrePost.scans, mdl.strength.groomPrePost.scans, mdl.degree.groomPrePost.scans, file = "GroomMdlOutput_PrePost_Scans.RData")
