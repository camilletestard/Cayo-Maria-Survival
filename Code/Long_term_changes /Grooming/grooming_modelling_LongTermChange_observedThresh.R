#grooming_modelling_LongTermChange_observedThresh.R
...
load("BisonGroomingScans_observedThresh.RData")
...
mdl.groomPrePost<-lmer( std.dens~ 1+ isPost.year + (1|group) , data=density.all)
summary(mdl.groomPrePost)
#performance::check_model(mdl.groomPrePost)

#Groom strength
mdl.strength.groomPrePost <- with(imp.prepost, lmer(std.groom.strength~ 1+ isPost.year +age+ sex +(1|group) + (1|id)))
mdl.pool.strength<-summary(mice::pool(mdl.strength.groomPrePost ))
...
#Groom degree
mdl.degree.groomPrePost <- with(imp, glmer(groom.degree~ 1+ isPost.year + sex +
                                 (1|group) + (1|id),
                               family=poisson(link = "log")))
mdl.pool.degree<-summary(mice::pool(mdl.degree.groomPrePost))

# #check model performance
# mdl.degree.groomPrePost.iter<-glmer(groom.degree~ 1+ isPost.year + sex +
#                                     (1|group) + (1|id),
...
setwd("~/Documents/GitHub/Cayo-Maria-Survival/Data/R.Data/")
save(mdl.groomPrePost, mdl.strength.groomPrePost, mdl.degree.groomPrePost, file = "GroomMdlOutput_PrePost_observedThresh.RData")
#load( "GroomMdlOutput_PrePost.RData")
