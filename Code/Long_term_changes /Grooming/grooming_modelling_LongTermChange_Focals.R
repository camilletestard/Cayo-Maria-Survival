  ...
  for (i in 1:num_iter){
    ...
    data.all = data.all %>%
      ...
      mutate(isPost.year = fct_relevel(isPost.year,
                                       "pre", "post.2019","post.2021","post2022"))
...
summary(mice::pool(mdl.strength.groomPrePost ))

#Groom degree
mdl.degree.groomPrePost <- with(imp, lmer(std.groom.degree~ 1+ isPost.year + sex +age+
                                            (1|group) + (1|id)))
summary(mice::pool(mdl.degree.groomPrePost))

setwd("~/Documents/GitHub/Cayo-Maria-Survival/Data/R.Data/")
save(mdl.groomPrePost, mdl.strength.groomPrePost, mdl.degree.groomPrePost, file = "GroomMdlOutput_PrePost_Focals.RData")
