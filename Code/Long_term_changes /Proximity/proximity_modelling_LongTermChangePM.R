...
groupyears = c("F2013","F2014","HH2014","F2015","V2015","R2015","KK2015",
...
mdl.proxPrePostPM<-lmer(std.dens~ 1+ isPost + (1|group) , data=density.all)
summary(mdl.proxPrePostPM)

#Proximity strength
mdl.strength.proxPrePostPM <- with(imp, lmer(std.prox.strength~ 1+ isPost +age+ sex 
                                           +(1|group) + (1|id)))
summary(mice::pool(mdl.strength.proxPrePostPM))

#Proximity degree
mdl.degree.proxPrePostPM <- with(imp, lmer(std.prox.degree~ 1+ isPost + sex +age+
                                           (1|group) + (1|id)))
summary(mice::pool(mdl.degree.proxPrePostPM))

#Save
save(mdl.proxPrePostPM, mdl.strength.proxPrePostPM, mdl.degree.proxPrePostPM, file = "ProxMdlOutput_PrePost_PM.RData")
