#proximity_modelling_LongTermChange_bigCayo.R
...
mdl.proxPrePost.BC<-lmer(std.dens~ 1+ isPost.year + (1|group), data=density.all)
summary(mdl.proxPrePost.BC)

#Proximity strength
mdl.strength.proxPrePost.BC <- with(imp, lmer(std.prox.strength~ 1+ isPost.year +age+ sex 
                                           +(1|group) + (1|id)))
summary(mice::pool(mdl.strength.proxPrePost.BC))

#Proximity degree
mdl.degree.proxPrePost.BC <- with(imp, lmer(std.prox.degree~ 1+ isPost.year + sex +age
                                           +(1|group) + (1|id)))
summary(mice::pool(mdl.degree.proxPrePost.BC))

#Save
save(mdl.proxPrePost.BC, mdl.strength.proxPrePost.BC, mdl.degree.proxPrePost.BC, file = "ProxMdlOutput_PrePost_bigCayo.RData")
#load( "ProxMdlOutput_PrePost_bigCayo.RData")
