#proximity_modelling_LongTermChange_smallCayo.R
...
mdl.proxPrePost.SC<-lm(std.dens~ 1+ isPost.year , data=density.all)
summary(mdl.proxPrePost.SC)

#Proximity strength
mdl.strength.proxPrePost.SC <- with(imp, lmer(std.prox.strength~ 1+ isPost.year +age+ sex 
                                           + (1|id) ))
summary(mice::pool(mdl.strength.proxPrePost.SC))

#Proximity degree
mdl.degree.proxPrePost.SC <- with(imp, lmer(std.prox.degree~ 1+ isPost.year + sex +age+
                                          (1|id) ))
summary(mice::pool(mdl.degree.proxPrePost.SC))

#Save
save(mdl.proxPrePost.SC, mdl.strength.proxPrePost.SC, mdl.degree.proxPrePost.SC, file = "ProxMdlOutput_PrePost_smallCayo.RData")
#load("ProxMdlOutput_PrePost_smallCayo.RData")
