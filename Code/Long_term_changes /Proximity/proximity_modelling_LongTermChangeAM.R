#proximity_modelling_LongTermChangeAM.R
...
mdl.proxPrePostAM<-lmer(std.dens~ 1+ isPost + (1|group) , data=density.all)
summary(mdl.proxPrePostAM)

#Proximity strength
mdl.strength.proxPrePostAM <- with(imp, lmer(std.prox.strength~ 1+ isPost +age+ sex 
                                           +(1|group) + (1|id)))
summary(mice::pool(mdl.strength.proxPrePostAM))

#Proximity degree
mdl.degree.proxPrePostAM <- with(imp, lmer(std.prox.degree~ 1+ isPost + sex +age+
                                           (1|group) + (1|id)))
summary(mice::pool(mdl.degree.proxPrePostAM))

#Save
save(mdl.proxPrePostAM, mdl.strength.proxPrePostAM, mdl.degree.proxPrePostAM, file = "ProxMdlOutput_PrePost_AM.RData")
