...
imputed.density=list()
  for (i in 1:num_iter){ #for each draw
    ...
    for (gy in 2:length(groupyears)){ #for each group year
    ...
    density.test$year.factor=as.factor(density.test$year)
...
mdl.aggPrePost<-lmer( std.dens~ 1+isPost.year + (1|group), data=density.all)
summary(mdl.aggPrePost)
performance::check_model(mdl.aggPrePost)

#2. Individual level aggression strength
mdl.strength.aggPrePost <- with(imp, lmer(std.agg.strength~ 1+ isPost.year + sex + age +(1|group) + (1|id)))
summary(mice::pool(mdl.strength.aggPrePost))

#per year
mdl.strength.aggPrePost.perYear <- with(imp, lmer(std.agg.strength~ 1+ year.factor + sex + age +(1|group) + (1|id)))
...
mdl.degree.aggPrePost <- with(imp, lmer(std.agg.degree~ 1+ isPost.year + sex +age+
                                            (1|group) + (1|id)))
summary(mice::pool(mdl.degree.aggPrePost))
...
save(mdl.aggPrePost, mdl.strength.aggPrePost.perYear, mdl.strength.aggPrePost,mdl.degree.aggPrePost , file = "AggMdlOutput_PrePost.RData")

load("AggMdlOutput_PrePost.RData")
