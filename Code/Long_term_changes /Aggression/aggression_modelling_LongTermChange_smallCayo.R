...
#Set group year list
group = c("V","V","V","V","V","V")
years = c(2015,2016,2017,2019,2021,2022)
groupyears = c("V2015","V2016","V2017","V2019","V2021","V2022")
...
mdl.aggPrePost.SC<-lm( std.dens~ 1+ isPost.year , data=density.all)
summary(mdl.aggPrePost.SC)
#performance::check_model(mdl.aggPrePost.SC)

#2. Individual level aggression strength
mdl.strength.aggPrePost.SC <- with(imp, lmer(std.agg.strength~ 1+ isPost.year + sex + age+
                                          +(1|id) ))
summary(mice::pool(mdl.strength.aggPrePost.SC))
...
mdl.degree.aggPrePost.SC <- with(imp, lmer(std.agg.degree~ 1+ isPost.year + sex +age+
                                             (1|id)))
summary(mice::pool(mdl.degree.aggPrePost.SC))
...
save(mdl.aggPrePost.SC, mdl.strength.aggPrePost.SC, mdl.degree.aggPrePost.SC , file = "AggMdlOutput_PrePost_smallCayo.RData")
