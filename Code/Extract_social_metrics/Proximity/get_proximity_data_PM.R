...
#Note: KK2013 does not have pm data
gy=7#16
...
for (gy in 1:length(groupyears)){ #for all group & years
  ...
  if (years[gy]==2018) {
   ...
   #Filter to only keep PM data
    scans2018<-scans2018[scans2018$timeBlock =="PM",]
    ...
    prox_data<-prox_data[prox_data$timeBlock =="PM",]
...
save(edgelist.all,file="proximity_data_PM.RData")
