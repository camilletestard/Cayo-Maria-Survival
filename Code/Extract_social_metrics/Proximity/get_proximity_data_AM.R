...
for (gy in 1:length(groupyears)){ #for all group & years
  ...
  if (years[gy]==2018) {
    ...
    scans2018$timeBlock[which(scans2018$start.time <= as_hms("10:00:00"))] = "AM";
    scans2018$timeBlock[which(scans2018$start.time > as_hms("11:00:00"))] = "PM";
    
   #Filter to only keep PM data
    scans2018<-scans2018[scans2018$timeBlock =="AM",]
    ...
    prox_data$time.parsed<-substr(prox_data$time, start = nchar(prox_data$time[1])-8, nchar(prox_data$time[1])-1)
    prox_data$time.parsed<-format(prox_data$time.parsed, format = "%H:%M:%S")
    prox_data$timeBlock[which(prox_data$time.parsed <= format("10:00:00", format = "%H:%M:%S" ))] = "AM";
    prox_data$timeBlock[which(prox_data$time.parsed > format("11:00:00", format = "%H:%M:%S" ))] = "PM";
    prox_data<-prox_data[prox_data$timeBlock =="AM",]
...
save(edgelist.all,file="proximity_data_AM.RData")
