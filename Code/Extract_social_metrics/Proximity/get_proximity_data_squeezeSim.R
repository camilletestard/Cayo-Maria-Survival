...
for (gy in 1:length(groupyears)){ #for all group & years
  ...
  df_obs_agg$total_obs_time = rowSums(cbind(df_obs_agg$ID1_obseff_duration, df_obs_agg$ID2_obseff_duration), na.rm=T)
  df_obs_agg$total_samples = rowSums(cbind(df_obs_agg$ID1_obseff_samples, df_obs_agg$ID2_obseff_samples), na.rm=T)
  
  #Remove individuals with NA total samples
  #df_obs_agg=df_obs_agg[df_obs_agg$total_samples!=0,]
  ...
  el=df_obs_agg[,c("ID1","ID2","count","total_samples")]; el$weight = el$count/el$total_samples
  el.nonzero=el[el$weight>0,]
  g = graph.data.frame(el.nonzero[,c("ID1","ID2","weight")])
  id_degree = igraph::degree(g)
  id_betweenness = betweenness(g)
  evc = eigen_centrality(g); id_EVC = evc$vector
  df_network_metrics=as.data.frame(t(rbind(id_degree, id_betweenness, id_EVC)))
  df_network_metrics$id = row.names(as.matrix(V(g)))
  df_network_metrics$year = years[gy]; df_network_metrics$group = group[gy]; df_network_metrics$obs="observed"
  
  den<-density(el$weight)
  plot(den, frame=F, col="blue", main="EdgeWeight")
  den<-density(df_network_metrics$id_degree)
  plot(den, frame=F, col="blue", main="Degree")
  den<-density(df_network_metrics$id_betweenness)
  plot(den, frame=F, col="blue", main="Betweenness")
  den<-density(df_network_metrics$id_EVC)
  plot(den, frame=F, col="blue", main="Eigenvector centrality")
  
  
  squeeze_param = 0.1 #10% increase in distance
  el.sim = el
  el.sim$weight = el.sim$weight + squeeze_param
