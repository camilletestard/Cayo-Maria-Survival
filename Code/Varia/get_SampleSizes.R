...
table(all_MetaData$group, all_MetaData$year)
...
#############################
# Get deaths per group & year
setwd("~/Documents/GitHub/Cayo-Maria-Survival/Data/R.Data/")
load('Survival_Adults_allGroups.RData') 

data = subset(SurvivalData,!duplicated(SurvivalData$id))
table(data$group)

deaths = data[data$Survival==1,]
table(deaths$group)

#############################################################################
#Get deaths considered for testing the survival effect of pre-hurricane social capital
...
#check who died...
dead.animals = SampleSize[SampleSize$Survival==1,]
# alive.animals = SampleSize[SampleSize$Survival==0,]
# hist(dead.animals$Age_entry.years)
# hist(dead.animals$degree); mean(dead.animals$degree)
# hist(alive.animals$degree); mean(alive.animals$degree)
# ggplot(SampleSize, aes(x=degree, fill=as.factor(Survival)))+
#   geom_histogram()
# table(dead.animals$sex)
# table(dead.animals$group)
