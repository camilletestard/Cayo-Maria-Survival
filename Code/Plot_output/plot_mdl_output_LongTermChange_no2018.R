...
load("GroomMdlOutput_PrePost_no2018.RData")#Longitudinal change in grooming
load("ProxMdlOutput_PrePost_no2018.RData")#Longitudinal change in proximity
...
output = mdl.density.prox.pool$coefficients; output = output[2:3,]
...
data$behavior = "proximity"; data$metric = "Standardized network density"; data$year = c(2019,2021)
...
output = mdl.density.groom.pool$coefficients; output = output[2:3,]
est = as.numeric(output[,1])
se = as.numeric(output[,2])
data=cbind(est, est-se*qnorm(0.975), est+se*qnorm(0.975))
data=data.frame(data); names(data)=c("estimate","lower","upper")
data$behavior = "grooming"; data$metric = "Standardized network density"; data$year = c(2019,2021)
data_combined= rbind(data_combined,data)
...
data$behavior = "proximity"; data$metric = "Standardized nodal strength"; data$year = c(2019,2021)
data_combined= rbind(data_combined,data)
...
data=cbind(est, est-se*qnorm(0.975), est+se*qnorm(0.975))
data=data.frame(data[3:4,]); names(data)=c("estimate","lower","upper")
data$behavior = "grooming"; data$metric = "Standardized nodal strength"; data$year = c(2019,2021)
data_combined= rbind(data_combined,data)
...
data=cbind(est, est-se*qnorm(0.975), est+se*qnorm(0.975))
data=data.frame(exp(data[2:3,])-1); names(data)=c("estimate","lower","upper")
data$behavior = "proximity"; data$metric = "Standardized nodal degree"; data$year = c(2019,2021)
data_combined= rbind(data_combined,data)
...
data=cbind(est, est-se*qnorm(0.975), est+se*qnorm(0.975))
data=data.frame(exp(data[3:4,])-1); names(data)=c("estimate","lower","upper")
data$behavior = "grooming"; data$metric = "Standardized nodal degree"; data$year = c(2019,2021)
data_combined= rbind(data_combined,data)

##################
# 4. Format output
##################
data_combined$year.after.storm <-ifelse(data_combined$year==2019, 2,4)
data_combined$year.after.storm = as.factor(data_combined$year.after.storm)
...
#Plot output
ggplot(data_combined, aes(x=year.after.storm,y=estimate)) +        # ggplot2 plot with confidence intervals
...
ggsave("Estimates_LongTermChange_no2018.pdf")
