#Aggression_survival_preTimeVarying_allgroups.R
...
#Set group year list
group = c("KK","F","HH","F","V","R","KK","R","V","F","HH","F","KK","V")
years = c(2013,2014,2014,2015,2015,2015,2015,2016,2016,2016,2016,2017,2017,2017)
groupyears = c("KK2013","F2014","HH2014",
...
# Goal: Test survival effects of post hurricane aggression

# #Standard status
# status.pre <- with(imp, coxme(Surv(Age_entry.days, Age_event.days, Survival)~
#                                       1+ sex + percofsex.dominanted+
#                                       (1|group) + (1|id) +(1|period)))
# summary(mice::pool(status.pre))
# 
# #Sex
...
# strength.agg.pre <- with(imp, coxme(Surv(Age_entry.days, Age_event.days, Survival)~ 
#                                       1+ std.node_strength+ sex + percofsex.dominanted+
#                                       (1|group) + (1|id) +(1|period)))
# mdl.pool.strength<-summary(mice::pool(strength.agg.pre))
...
INstrength.agg.pre <- with(imp, coxme(Surv(Age_entry.days, Age_event.days, Survival)~ 
                                         1+ std.node_INstrength+ sex + #percofsex.dominanted+
                                         (1|group) + (1|id) +(1|period)))
mdl.pool.INstrength<-summary(mice::pool(INstrength.agg.pre))
...
OUTstrength.agg.pre <- with(imp, coxme(Surv(Age_entry.days, Age_event.days, Survival)~ 
                                          1+ std.node_OUTstrength+ sex + #percofsex.dominanted+
                                          (1|group) + (1|id) +(1|period)))
mdl.pool.OUTstrength<-summary(mice::pool(OUTstrength.agg.pre))
...
# degree.agg.pre <- with(imp, coxme(Surv(Age_entry.days, Age_event.days, Survival)~ 
#                            1+ std.degree+ sex+ percofsex.dominanted+
#                            (1|group) + (1|id) +(1|period)))
# mdl.pool.degree<-summary(mice::pool(degree.agg.pre))
...
save(INstrength.agg.pre, OUTstrength.agg.pre, strength.agg.pre, degree.agg.pre, file = "AggMdlOutput_PreHurr_allgroups.RData")
# setwd("~/Documents/GitHub/Cayo-Maria-Survival/Data/R.Data/")
# load("AggMdlOutput_PreHurr_allgroups.RData")
