...
for (i in 1:num_iter){
  ...
  data.final<-data.final[which(data.final$period==6 & data.final$year==2018|
                     data.final$period==7 & data.final$year==2019|
...
degree.prox.post.SC <- with(imp, coxme(Surv(Age_entry.days, Age_event.days, Survival)~ 
                           1+ std.degree+ sex+ percofsex.dominanted+
                           (1|group) + (1|id) +(1|period)))
mdl.pool.degree<-summary(mice::pool(degree.prox.post.SC))
...
strength.prox.post.SC <- with(imp, coxme(Surv(Age_entry.days, Age_event.days, Survival)~ 
                                      1+ std.node_strength+ sex + percofsex.dominanted+
                                      (1|group) + (1|id) +(1|period)))
mdl.pool.strength<-summary(mice::pool(strength.prox.post.SC))
...
save(strength.prox.post.SC, degree.prox.post.SC, file = "ProxMdlOutput_PostHurr_smallCayo.RData")
# setwd("~/Documents/GitHub/Cayo-Maria-Survival/Data/R.Data/")
