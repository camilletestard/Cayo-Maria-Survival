...
for (gy in 1:length(groupyears)){
  ...
  SocialCapitalData$Age_event.days = SocialCapitalData$DOD- SocialCapitalData$DOB #event = death
  SocialCapitalData$Age_event.days[!is.na(SocialCapitalData$DOT)] = #event = removal
    SocialCapitalData$DOT[!is.na(SocialCapitalData$DOT)]- SocialCapitalData$DOB[!is.na(SocialCapitalData$DOT)]
  SocialCapitalData$Age_event.days[is.na(SocialCapitalData$Age_event.days)] = #event = end of study
    SocialCapitalData$Age_entry.days[is.na(SocialCapitalData$Age_event.days)] + study.perod 
  ...
  SocialCapitalData$Age_event.days[which(SocialCapitalData$DOT>SocialCapitalData$study.end.date[1])] = SocialCapitalData$Age_entry.days[which(SocialCapitalData$DOT>SocialCapitalData$study.end.date[1])] + study.perod
  ...
  #Remove individuals that were dead or transfered before the start of the study
  idx_to_remove = which(SocialCapitalData$DOD<study.start.date | SocialCapitalData$DOT<study.start.date)
  SocialCapitalData$Status[idx_to_remove]=NA
  SocialCapitalData<- SocialCapitalData %>%  filter(!is.na(Status))
  ...
  #Get year of death
  SocialCapitalData$YearOfDeath = year(SocialCapitalData$DOD)
  SocialCapitalData$YearOfDeath[SocialCapitalData$Survival==0]=NA
...
SocialCapital.ALL$YearOfEvent[is.na(SocialCapital.ALL$YearOfEvent)] = 2022

#Get year of death
SocialCapital.ALL$YearOfDeath = year(SocialCapital.ALL$DOD)
...
allids = unique(SocialCapital.ALL$id); 
unique_SocialCapital = SocialCapital.ALL[match(allids, SocialCapital.ALL$id),]
table(unique_SocialCapital$group); table(unique_SocialCapital$Survival)
table(unique_SocialCapital$group, unique_SocialCapital$Survival)

table(unique_SocialCapital$group, unique_SocialCapital$YearOfDeath)
