...
for (i in 1:max_iter){
  ...
  try(fitsocial.groom<-coxme(Surv(Age_entry.days, Age_event.days, Survival)~dpSocial.cat +group +sex +(1|id)+(1|year.prehurr),data=data)) #Runs a cox PH model with age as the time scale.
  summary(fitsocial.groom)
  # cz <- cox.zph(fitsocial.groom)
  # print(cz)
  
  try(fitsocial.prox<-coxme(Surv(Age_entry.days, Age_event.days, Survival)~dpAcc.cat +group +sex +(1|id) + (1|year.prehurr),data=data)) #Runs a cox PH model with age as the time scale.
...
idx.outlier.prox = which(abs(fit.prox.hr) > 2, arr.ind = T)
