#bison_proximity_survival_prePost.R

#1. Model proximity networks from before and after the hurricane based on scan data 
#and extract change in proximity pre-to-post using bison.
...
########################################################################
## STEP 1: Fit network to observed proximity binary data susing BiSON###
########################################################################
...
for (gy in 1:length(groupyears)){ #for years
  ...
  el.pre$weight=el.pre$count/el.pre$total_samples
  ...
  el.post$weight=el.post$count/el.post$total_samples
  ...
  #Draw n edgelists from the posterior of the networks
  ...
  for(i in 1:num_iter){
    ...
    weightedEL[weightedEL[,3]<0.001,3]=0
    #weightedEL[which(el.pre$weight==0),3]=0
    ...
    weightedEL[weightedEL[,3]<0.001,3]=0
    # weightedEL[which(el.post$weight==0),3]=0
...
save(node_ids, node_strength_change, node_degree_change, num_iter, file = "ProxChange.RData")


######################################
## STEP 2: Create imputed data set ###
######################################
#Imputed data set from the distribution of networks in bison.
...
degree.dprox <- with(imp, coxme(Surv(Age_entry.days, Age_event.days, Survival)~ 
                                          1+ std.d.degree + sex+ group+
                                          (1|year) + (1|id)) )
summary(mice::pool(degree.dprox))
#NOTE: Model won't run for change in degree with id as a random effect. If remove (1|id) then it runs

strength.dprox <- with(imp, coxme(Surv(Age_entry.days, Age_event.days, Survival)~ 
                                          1+ std.d.strength + sex+ group+
                                          (1|year) +(1|id)))
...
setwd("~/Documents/GitHub/Cayo-Maria-Survival/Data/R.Data/")
save(strength.dprox, file = "MdlOutput_ChangeProximity.RData")
